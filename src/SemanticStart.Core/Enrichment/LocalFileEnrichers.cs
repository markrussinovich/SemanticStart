...
public sealed class MsixManifestEnricher : IEnricher
{
    ...
    private static async Task<string?> ResolveInstallLocationAsync(string packageFamilyName, CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(ps))
        {
            var script = $"$p=Get-AppxPackage -PackageFamilyName '{packageFamilyName.Replace("'", "''")}' -ErrorAction SilentlyContinue; if ($p) {{ $p.InstallLocation }}";
            var output = await ProcessRunner.RunAsync(ps, ["-NoProfile", "-NonInteractive", "-Command", script], TimeSpan.FromSeconds(3), cancellationToken).ConfigureAwait(false);
            var path = output.Output.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
            if (!string.IsNullOrWhiteSpace(path) && Directory.Exists(path.Trim())) return path.Trim();
        }
        try
        {
            var root = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "WindowsApps");
            if (Directory.Exists(root))
                return Directory.EnumerateDirectories(root, packageFamilyName + "*", SearchOption.TopDirectoryOnly).FirstOrDefault(d => File.Exists(Path.Combine(d, "AppxManifest.xml")));
        }
